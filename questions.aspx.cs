...
public partial class questions : System.Web.UI.Page
{
    ...
    public int DisplayRepeter()
    {
        ...
        dt = verma.Show_Data("Select * from interview_questions where department_id='" + Page.RouteData.Values["id"] + "'");
        ...
        QuestionsRepeater.DataSource = pds;
        QuestionsRepeater.DataBind();
    ...
    protected void search_submit_Click(object sender, EventArgs e)
    {
        if (search_text.Text != "")
        {
            QuestionsRepeater.DataSource = verma.Show_Data("select * from interview_questions where question like '%" + search_text.Text + "%' or answer like '%" + search_text.Text + "%' and status=1 order by position");
            QuestionsRepeater.DataBind();
        }
        else
        {
            QuestionsRepeater.DataSource = verma.Show_Data("Select * from interview_questions where department_id=" + Page.RouteData.Values["id"] + "'");
            QuestionsRepeater.DataBind();
        }
    }


    protected void show_answer_Command(object sender, CommandEventArgs e)
    {
        ClientScript.RegisterStartupScript(this.GetType(), "Popup", "ShowAnswer();", true);
    }
}